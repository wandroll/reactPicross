...
const prepareDataSet = (dataSet: number[]): Item [] => dataSet.map(value => ({ isFilled: (value === 1) }))
 ...
 describe(' computeCellsIndication', () => {
    ...
    it('should return the indications for a list of cells', () => {
        ...
        expect( computeCellsIndication(prepareDataSet(dataSet))).toEqual(ecpectedOutput);
    })
})